...
import { Container, Title, Text, LoadingOverlay, Modal, Button } from '@mantine/core';
import {
    User,
    Ticket,
    HelpCircle,
...
const ProfileView: React.FC = () => {
    ...
    useEffect(() => {
        const loadUserData = async () => {
            try {
                ...
                console.log('Datos de la Tarjeta de Propiedad recibidos:', propertyCardData);
    ...
    useEffect(() => {
        const checkDocumentCompletion = async () => {
               if (userProfile) {
                  const allDocumentsComplete = vehicleStatus.hasVehicle &&
                 ...
                 if (userProfile.user_type !== "DRIVER" && allDocumentsComplete) {
                      try {
                        ...
                        const userId = localStorage.getItem('userId');
                          ...
                          if (!token || !userId) {
                             console.error('No hay token o userId disponible para cambiar el rol del usuario.');
                              return;
                          }
                        ...
                        console.log("Actualizando user_type a DRIVER:", requestBody, 'URL', updateUrl);
                        ...
                        if (!response.ok) {
                          ...
                          console.error('Error al actualizar el rol del usuario:', errorData);
                        ...
                        setSuccessMessage(`Felicidades ${userProfile.first_name} ${userProfile.last_name} ya tienes las habilidades de conductor en cupo.`);
    ...
    const renderVehicleSubmenu = () => (
        <div className={styles.subMenu}>
            ...
            {/* Completion message */}
           {showVehicleMessage && !vehicleStatus.hasVehicle &&
                !vehicleStatus.hasLicense &&
                !vehicleStatus.hasSoat &&
                !vehicleStatus.hasPropertyCard &&(
                  <div className={styles.vehicleIncompleteMessage}>
                        <Text className={styles.vehicleIncompleteText}>Para ser conductor, debes completar todos los registros</Text>
                  </div>
              )}
               {showVehicleMessage &&  (vehicleStatus.hasVehicle || vehicleStatus.hasLicense || vehicleStatus.hasSoat || vehicleStatus.hasPropertyCard ) &&(
                <div className={styles.vehicleIncompleteMessage}>
                    <Text className={styles.vehicleIncompleteText}>Para ser conductor debes completar todos los registros del vehiculo</Text>
                </div>
               )}
        ...
        <Container fluid className={styles.container}>
            ...
            <div className={styles.userSection}>
                ...
                <div className={styles.userInfo}>
                    ...
                    <Text className={`${styles.userType} ${userProfile?.user_type === 'DRIVER' ? styles.driver : ''}`}>
